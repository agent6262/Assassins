...
/**
 * Base class that represents a row from the 'presets' table.
 *
 * 
 *
 * @package    propel.generator..Base
 */
abstract class Preset implements ActiveRecordInterface 
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\PresetTableMap';
    ...
    /**
     * @var        ChildSetting one-to-one related ChildSetting object
     */
    protected $singleSetting;
    ...
    /**
     * Initializes internal state of Base\Preset object.
     */
    ...
    /**
     * Compares this with another <code>Preset</code> instance.  If
     * <code>obj</code> is an instance of <code>Preset</code>, delegates to
     * <code>equals(Preset)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|Preset The current object, for fluid interface
     */
     ...
     * @return $this|\Preset The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[PresetTableMap::COL_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[PresetTableMap::COL_NAME] = true;
        }
    ...
    public function setRules($v)
    {
        ...
        if ($this->rules !== $v) {
            $this->rules = $v;
            $this->modifiedColumns[PresetTableMap::COL_RULES] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : PresetTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PresetTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PresetTableMap::translateFieldName('Rules', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = PresetTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Preset'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildPresetQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see Preset::setDeleted()
     * @see Preset::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildPresetQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                PresetTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->singleSetting !== null) {
                if (!$this->singleSetting->isDeleted() && ($this->singleSetting->isNew() || $this->singleSetting->isModified())) {
                    $affectedRows += $this->singleSetting->save($con);
                }
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[PresetTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . PresetTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(PresetTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(PresetTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = '`name`';
        }
        if ($this->isColumnModified(PresetTableMap::COL_RULES)) {
            $modifiedColumns[':p' . $index++]  = '`rules`';
        }

        $sql = sprintf(
            'INSERT INTO `presets` (%s) VALUES (%s)',
    ...
    {
        $pos = PresetTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Preset'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Preset'][$this->hashCode()] = true;
        $keys = PresetTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getName(),
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->singleSetting) {
                ...
                $result[$key] = $this->singleSetting->toArray($keyType, $includeLazyLoadColumns, $alreadyDumpedObjects, true);
            }
        }
    ...
    {
        $criteria = new Criteria(PresetTableMap::DATABASE_NAME);

        if ($this->isColumnModified(PresetTableMap::COL_ID)) {
            $criteria->add(PresetTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(PresetTableMap::COL_NAME)) {
            $criteria->add(PresetTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(PresetTableMap::COL_RULES)) {
            $criteria->add(PresetTableMap::COL_RULES, $this->rules);
        }
    ...
    {
        $criteria = ChildPresetQuery::create();
        $criteria->add(PresetTableMap::COL_ID, $this->id);
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            $relObj = $this->getSetting();
            if ($relObj) {
                $copyObj->setSetting($relObj->copy($deepCopy));
            }
     ...
     * If this ChildPreset is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildPreset The current object (for fluent API support)
     */
    public function setUserPresets(Collection $userPresets, ConnectionInterface $con = null)
    {
        ...
        foreach ($userPresetsToDelete as $userPresetRemoved) {
            $userPresetRemoved->setPreset(null);
        }
     ...
     * Otherwise if this Preset is new, it will return
     * an empty collection; or if this Preset has previously
     * been saved, it will retrieve related UserPresets from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Preset.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     ...
     * @return ObjectCollection|ChildUserPreset[] List of ChildUserPreset objects
     */
    public function getUserPresetsJoinUser(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildUserPresetQuery::create(null, $criteria);
    ...
    public function setPresetGroups(Collection $presetGroups, ConnectionInterface $con = null)
    {
        ...
        foreach ($presetGroupsToDelete as $presetGroupRemoved) {
            $presetGroupRemoved->setPreset(null);
        }
     ...
     * @return ObjectCollection|ChildPresetGroup[] List of ChildPresetGroup objects
     */
    public function getPresetGroupsJoinGroup(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildPresetGroupQuery::create(null, $criteria);
    ...
    /**
     * Gets a single ChildSetting object, which is related to this object by a one-to-one relationship.
     *
     * @param  ConnectionInterface $con optional connection object
     * @return ChildSetting
     * @throws PropelException
    ...
    public function getSetting(ConnectionInterface $con = null)
    {
        ...
        if ($this->singleSetting === null && !$this->isNew()) {
            $this->singleSetting = ChildSettingQuery::create()->findPk($this->getPrimaryKey(), $con);
        }
    ...
    /**
     * Sets a single ChildSetting object as related to this object by a one-to-one relationship.
     *
     * @param  ChildSetting $v ChildSetting
     * @return $this|\Preset The current object (for fluent API support)
     * @throws PropelException
     */
    public function setSetting(ChildSetting $v = null)
    {
        $this->singleSetting = $v;

        // Make sure that that the passed-in ChildSetting isn't already associated with this object
        if ($v !== null && $v->getPreset(null, false) === null) {
            $v->setPreset($this);
        }
     ...
     * @return ChildPreset The current object (for fluent API support)
     */
    ...
    protected function doAddUser(ChildUser $user)
    {
        ...
        if (!$user->isPresetsLoaded()) {
            $user->initPresets();
            $user->getPresets()->push($this);
        } elseif (!$user->getPresets()->contains($this)) {
            $user->getPresets()->push($this);
        }
    ...
    {
        if ($this->getUsers()->contains($user)) { $userPreset = new ChildUserPreset();

            $userPreset->setUser($user);
            if ($user->isPresetsLoaded()) {
                //remove the back reference if available
                $user->getPresets()->removeObject($this);
            }
    ...
    protected function doAddGroup(ChildGroup $group)
    {
        ...
        // set the back reference to this object directly as using provided method either results
        // in endless loop or in multiple relations
        if (!$group->isPresetsLoaded()) {
            $group->initPresets();
            $group->getPresets()->push($this);
        } elseif (!$group->getPresets()->contains($this)) {
            $group->getPresets()->push($this);
        }
    ...
    {
        if ($this->getGroups()->contains($group)) { $presetGroup = new ChildPresetGroup();

            $presetGroup->setGroup($group);
            if ($group->isPresetsLoaded()) {
                //remove the back reference if available
                $group->getPresets()->removeObject($this);
            }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->singleSetting) {
                $this->singleSetting->clearAllReferences($deep);
            }
            if ($this->collUsers) {
    ...
    {
        return (string) $this->exportTo(PresetTableMap::DEFAULT_STRING_FORMAT);
    }
